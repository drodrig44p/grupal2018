...
En los 9 cuadrantes muestreados, se encontraron 42 individuos de la especie *Lobatus galeatus* en la zona rocosa intermareal muestreada. Todos fueron categorizados como juveniles (< 15 cm) (Figura ). Solamente dos de los individuos tuvieron longitudes de la concha mayores de 6 cm, la mayoría fueron menor a 3 cm de largo. 


```{r, fig.cap='Longitud promedio de L. galeatus por remoción'}
...
stackdir = "center",alpha=0.7)+ theme_classic()+ labs(x = "Remoción", y = "Longitud de la concha (cm)")
```

En cada cuadrante, se encontraron menos individuos en la remoción 3 comparada con la 1. 
```{r,include=FALSE}
library(ggplot2)
datosyoss <- read.delim("./Juveniles_2019.csv",h=T,sep = ";")
```

```{r, fig.cap='presencia de individuos juveniles de *L. galeatus* por remoción'}
ggplot(data = datosyoss , aes(x = Remocion, y = Presencia)) +  geom_bar(stat = "identity", fill="salmon")+ theme_classic(
)
```
...
```{r,include=FALSE}
library(unmarked)
juvenil <- read.csv("./Juveniles2019.csv", h=F, row.names= NULL, sep= ";")
...
Los resultados estiman 2.53 individuos por cuadrante, con una probabilidad de detección 
```{r}
