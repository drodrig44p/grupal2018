...
# 
# 
# (m3 <- gmultmix(~1, ~1,~1, data=umfGMM1, K=30)) #no hay covariables 
# backTransform(m2, type="lambda") # Individuals per plot
# backTransform(m3, type="phi") # Probability of being avilable
# (p <- backTransform(m3, type="det")) # Probability of detection
# p <- coef(p)
# # Multinomial cell probabilities under removal design
# c(p, (1-p) * p, (1-p)^2 * p)
# # Or more generally:
